...
scripts/verify-layout \
  /home/chabking/go/bin/kratos \
  .tools/bin/buf \
  .tools/bin/govulncheck \
  .tools/bin/cyclonedx-gomod \
  .tools/bin/gitleaks
```
...
| V12 | executable policy scan, rooted inside the private candidate clone, found no moving `@latest`, Todo, Wire, Ent, AIP, business API, workspace, submodule, or replace residue in active source/build paths | pass |
...
| V15 | a second ordinary ANI module and legal edge names `go`, `ani-service-template`, `ani-kratos-layout`, `todo`, and `ani-todo-service` produced valid independent repositories; `go` preserved `go/parser` and `go/token` while normalizing owned imports | pass |
...
| V17 | generated repositories retained service README/AGENTS/runtime docs/CI/provenance and executable supply-chain verification, while omitting the layout-only wrapper, templates, evidence, and workflow | pass |
...
| V54 | inside the aggregate gate and with layout source unavailable, a newly generated service was committed, passed `make audit`, committed its own BOM, then passed the same audit and CI freshness assertions again with a clean tree | pass |
